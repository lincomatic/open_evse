...
void J1772EVSEController::Update()
{
  ...
  if (m_EvseState == EVSE_STATE_C) {
	...
	SetCurrentCapacity(g_TempMonitor.m_ampacity / 2,0);     // set L2 to the throttled back level
      }
	...
	SetCurrentCapacity(g_TempMonitor.m_ampacity,0);    // set L2 to the user's original setting for L2 current
      }           
	...
	g_TempMonitor.SetOverTemperatureShutdown(1);
        SetCurrentCapacity(g_TempMonitor.m_ampacity / 4,0);
      }
